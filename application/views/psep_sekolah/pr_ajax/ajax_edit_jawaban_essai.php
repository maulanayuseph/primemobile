	...
	<div class="input_fields_wrap">
		<div class="form-group">
			<div class="col-md-12">
				...
				<textarea><?php echo $soal->jawaban;?></textarea>
			</div>
		</div>
	</div>
	<div class="col-sm-12">
...
$(function(){
	$("#simpan-soal").click(function(){
		...
		jawaban		= tinyMCE.activeEditor.getContent();
		$.ajax({
			type: 'POST',
			url: '../ajax_proses_edit_jawaban_essai',
			data:{
				'idsoal'		: idsoal,
				'jawaban'		: jawaban
			}
	...
	$(document).ajaxSend(function(event, jqxhr, settings){
		if(settings.url === "../ajax_proses_edit_jawaban_essai"){
			$('#text-load').html('Menyimpan soal');
			$('#modal-loader').appendTo("body").modal('show');
		}
	});
	$(document).ajaxSuccess(function(event, request, options){
		if(options.url === "../ajax_proses_edit_jawaban_essai"){
