...
<html lang="en">
    ...
    <?php include "modal_pembahasan.php"; ?>
		...
		<!-- Page Body -->
	<form id="form_soal" method="post" action="<?php echo base_url('pr/penilaian')?>">
		<div class="page-body latihan-soal">
			...
			<div class="soal-header">
				...
				<ul class="keterangan">
					<li>
						<button type="submit" name="submit" class="btn btn-primary"  onclick="return confirm('Apakah anda yakin untuk submit tugas?')"><span class="glyphicon glyphicon-send" aria-hidden="true"></span></button>
					</li>
				</ul></div>
			</div>
					...
					<div class="container">
							...
							if(isset($terjawab)){
								foreach($terjawab as $sudah_jawab){
									if($sudah_jawab->id_soal_pr == $item->id_soal_pr){
										$sudah_dijawab[$item->id_soal_pr] = $sudah_jawab->terjawab;
									}
								}
							}
							?>
								...
								<div id="item_soal_<?php echo $no;?>" class="tab-pane fade">
									...
									<div class="row equal-row item-soal">
										...
										<div class="col-sm-8 equal-col jawaban">
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-a_<?php echo $no;?>" value="<?php echo $item->id_soal_pr; ?>-1" 
											<?php 
												if(isset($sudah_dijawab[$item->id_soal_pr])){
													if($sudah_dijawab[$item->id_soal_pr] == 1){
														echo "checked";
													}
												}
											?>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-b_<?php echo $no;?>" value="<?php echo $item->id_soal_pr; ?>-2"
											<?php 
												if(isset($sudah_dijawab[$item->id_soal_pr])){
													if($sudah_dijawab[$item->id_soal_pr] == 2){
														echo "checked";
													}
												}
											?>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-c_<?php echo $no;?>" value="<?php echo $item->id_soal_pr; ?>-3"
											<?php 
												if(isset($sudah_dijawab[$item->id_soal_pr])){
													if($sudah_dijawab[$item->id_soal_pr] == 3){
														echo "checked";
													}
												}
											?>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-d_<?php echo $no;?>" value="<?php echo $item->id_soal_pr; ?>-4"
											<?php 
												if(isset($sudah_dijawab[$item->id_soal_pr])){
													if($sudah_dijawab[$item->id_soal_pr] == 4){
														echo "checked";
													}
												}
											?>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-e_<?php echo $no;?>" value="<?php echo $item->id_soal_pr; ?>-5"
											<?php 
												if(isset($sudah_dijawab[$item->id_soal_pr])){
													if($sudah_dijawab[$item->id_soal_pr] == 5){
														echo "checked";
													}
												}
											?>
					...
					<div class="soal-pagination">
						...
						<nav class="text-center">
						  ...
						  <ul id="toggle_soal" class="pagination custom-pagination" style="overflow-x: scroll">
						    ...
						    { ?>
							<?php 
								if(isset($sudah_dijawab[$page->id_soal_pr])){
							?>
								<li class="benar">
    ...
    <script type="text/javascript">
...

    	{
			...
			$("#pembahasan" + id_opt[1]).removeClass('hidden');   
				...
				$.post("<?=base_url('pr/ajax_check_jawaban');?>",
    ...
    <script type="text/javascript">
...

    	{
    		...
    		$.post("<?=base_url('pr/ajax_fetch_pembahasan');?>",
