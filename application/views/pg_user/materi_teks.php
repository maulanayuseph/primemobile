    ...
    <script type="text/javascript">
      ...
      $(document).ready(function(){
      	$(document).ajaxSend(function(event, jqxhr, settings){
			if(settings.url === "<?=base_url('materi/ajax_change_content');?>"){
				$('#judul_materi').hide();
		        $('#isi_materi').hide();
		        $("#ajax-loading").show();
			}
		...
		$(document).ajaxSuccess(function(event, request, options){
			if(options.url === "<?=base_url('materi/ajax_change_content');?>"){
				$("#ajax-loading").fadeOut(100);
			}
		});

		$('div').each(function(){
			classvideo = $(this).attr("class");
			var element = this;
			if(classvideo === "video-smil"){
				urlvideo = $(this).html();
				$.ajax({
					type: 'POST',
					url: '../../ajax_view_video',
