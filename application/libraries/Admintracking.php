...
class Admintracking
{
  ...
  private $needed_fields = array(array('name' => 'id', 'type' => 'int', 'primary_key' => 1, 'forge_type' => 'int', 'forge_auto_increment' => TRUE),
                            array('name' => 'session_id', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '100'),
														array('name' => 'sub_materi_id', 'type' => 'int', 'forge_type' => 'int', 'forge_constraint' => '11'),
														...
														array('name' => 'timestamp', 'type' => 'string', 'forge_type' => 'varchar', 'forge_constraint' => '50'),
  ...
  // --------------------------------------------------------------------
  /**
   * Personally edited session (ANADRIV)
   *
   */
  public function track_this_session($id_konten=NULL)
  {
    //check the conditions
    if (($this->CI->session->userdata('id_siswa')!=NULL) && ($this->CI->session->userdata('akses')!=NULL)) {
        return $this->track_this($id_konten);
    }
    else {
        return false;
    }
   ...
   * @return boolean The result of the tracking action (whether a db record was added or not)
   */
  public function track_this($id_konten=NULL)
  {
    //load necessary libraries
    $this->CI->load->database();
    $this->CI->load->library('user_agent');
    $this->CI->load->library('session');
    $sub_materi_id = explode('/', $this->CI->input->server('REQUEST_URI'));

    //get the data
    $input_data = array();
    $input_data['session_id'] = ($this->CI->session->userdata('id_siswa') != NULL) ? $this->CI->session->userdata('id_siswa') : 0;
    // $input_data['sub_materi_id'] = (!empty($id_konten)) ? $id_konten : 0;
    $input_data['sub_materi_id'] = end($sub_materi_id);
    $input_data['request_uri'] = $this->CI->input->server('REQUEST_URI');
    $input_data['timestamp'] = date("Y-m-d H:i:00");
