...
class Model_adm extends CI_Model
{
	...
	function add_materi($kategori, $mapel_id, $materi_pokok_id, $nama_sub_materi, $deskripsi_sub_materi, $isi_materi, $video_materi, $gambar_materi, $tanggal, $waktu, $urutan_materi, $kurikulum, $statuskonten, $catatanrevisi)
	{	
	...
	function update_materi($id, $kategori, $mapel_id, $materi_pokok_id, $nama_sub_materi, $deskripsi_sub_materi, $isi_materi, $video_materi, $gambar_materi, $tanggal, $waktu, $kurikulum, $statuskonten, $catatanrevisi)
	{
	...
	function update_materi_pokok($id, $mapel_id, $nama_materi_pokok, $judulk13, $judulktsp, $deskripsi_materi_pokok)
	{
	...
	function add_item_soal($isi_soal, $jawab_1, $jawab_2, $jawab_3, $jawab_4, $jawab_5, $kunci_jawaban, $sub_materi_id, $pembahasan, $pembahasan_video,$bobot, $idadm)
	{	
		//Insert data into table soal 
		$data = array(
			'isi_soal' 				=> $isi_soal,
		...
		//backup soal di history before
		$this->db->insert('qc_history', $datahistory);
		
		return $idsoal;
	}

	function update_item_soal($isi_soal,$jawab_1,$jawab_2,$jawab_3,$jawab_4,$jawab_5,$kunci_jawaban,$pembahasan,$pembahasan_video,$id_soal,$bobot, $status)
	{
		
		//UPDATE HISTORY UNTUK PEREKAPAN QC
		//*********************************
		//*********************************
		//*********************************
		
		//fetch soal lama
		$soallama = $this->fetch_soal_by_id($id_soal);
		...
		$historybefore = array(
			"id_qc_history"		=> $idhistory,
			"soal"				=> $soallama->isi_soal,
			"status"			=> $soallama->status,
			"jawab_1"			=> $soallama->jawab_1,
			"jawab_2"			=> $soallama->jawab_2,
			"jawab_3"			=> $soallama->jawab_3,
			"jawab_4"			=> $soallama->jawab_4,
			"jawab_5"			=> $soallama->jawab_5,
			"kunci_jawaban"		=> $soallama->kunci_jawaban,
			"pembahasan"		=> $soallama->pembahasan,
			"pembahasan_video"	=> $soallama->pembahasan_video,
			"bobot"				=> $soallama->bobot
		);
		$this->db->insert('qc_history_before', $historybefore);
		
		//record di history after
		$historyafter = array(
			"id_qc_history"		=> $idhistory,
			"soal"				=> $isi_soal,
			"status"			=> $status,
		...
		$this->db->insert('qc_history_after', $historyafter);
		//END UPDATE HISTORY UNTUK PEREKAPAN QC
		//*********************************
		//*********************************
		//*********************************
	...
	function fetch_all_pembayaran(){
		$this->db->select("pembelian.no_tagihan, pembelian.siswa_id, pembelian.total_harga, pembelian.metode_pembayaran, pembelian.status, pembelian.status as angka_status, pembelian.expired_on, pembelian.id_pembelian, pembelian.file_bukti, pembelian.status, pembelian.email, siswa.nama_siswa, siswa.email as email_siswa");
	...
	$penyelenggara, $tgl, $jam, $biaya, $banner, $tanggalpost, $waktupost, $tipe, $idpaket, $jenis, $enddate){
		$profil = array(
			'id_kelas' 		=> $kelas,
			'nama_profil' 	=> $nama,
...
//model untuk rekapitulasi konten
//#########################################
//#########################################
//#########################################
...
function jumlah_soal_bab($idkelas){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok");
	$this->db->join("mata_pelajaran", "materi_pokok.mapel_id=mata_pelajaran.id_mapel");
	$this->db->where("mata_pelajaran.kelas_id", $idkelas);
	$this->db->like("sub_materi.nama_sub_materi", "LATIHAN SOAL BAB","after");
	
	$result = $this->db->count_all_results();
	return $result;
}
function jumlah_soal_bab2($idkelas){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok");
	$this->db->join("mata_pelajaran", "materi_pokok.mapel_id=mata_pelajaran.id_mapel");
	$this->db->where("mata_pelajaran.kelas_id", $idkelas);
	$this->db->like("sub_materi.nama_sub_materi", "LATIHAN SOAL CHAPTER", "after");
	
	$result = $this->db->count_all_results();
	return $result;
}
function jumlah_soal_bab3($idkelas){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok");
	$this->db->join("mata_pelajaran", "materi_pokok.mapel_id=mata_pelajaran.id_mapel");
	$this->db->where("mata_pelajaran.kelas_id", $idkelas);
	$this->db->where("sub_materi.nama_sub_materi", "LATIHAN SOAL");
	
	$result = $this->db->count_all_results();
	return $result;
}

function jumlah_pembahasan_video_by_kelas($idkelas){
	$this->db->select("*");
	$this->db->from("jawaban");
	...
	$this->db->join("mata_pelajaran", "materi_pokok.mapel_id=mata_pelajaran.id_mapel");
	$this->db->where("mata_pelajaran.kelas_id", $idkelas);
	$this->db->like("jawaban.pembahasan_video", "http://stream.primemobile.co.id:1935/TestVOD", "after");
...
function fetch_materi_pokok_by_mapel2($idmapel){
	$this->db->select("*");
	$this->db->from("materi_pokok");
...
function jumlah_pembahasan_video_by_sub_materi($idsubmateri){
	$this->db->select("*");
	$this->db->from("jawaban");
	$this->db->join("soal", "jawaban.soal_id=soal.id_soal");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi");
...
//end model untuk rekapitulasi konten
//#########################################
//#########################################
//#########################################

//MODEL UNTUK CETAK PDF LATIHAN SOAL !!! 
//#########################################
//#########################################
//#########################################

function fetch_sub_materi_by_kelas($idkelas){
	$this->db->select("*");
	$this->db->from("konten_materi");
...
function fetch_soal_by_sub_materi_non_penulis($idsubmateri){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("jawaban", "soal.id_soal = jawaban.soal_id", "left");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi", "left");
...
function fetch_sub_by_id($idsubmateri){
	$this->db->select("*");
	$this->db->from("sub_materi");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok", "left");
	$this->db->join("mata_pelajaran", "materi_pokok.mapel_id = mata_pelajaran.id_mapel", "left");
...
function fetch_sub_soal_by_mapel($idmapel){
	$this->db->select("*");
	$this->db->from("konten_materi");
	$this->db->join("sub_materi", "konten_materi.sub_materi_id = sub_materi.id_sub_materi", "left");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok", "left");
...
function fetch_sub_soal_by_materi_pokok($idmateripokok){
	$this->db->select("*");
	$this->db->from("konten_materi");
	$this->db->join("sub_materi", "konten_materi.sub_materi_id = sub_materi.id_sub_materi", "left");
	$this->db->join("materi_pokok", "sub_materi.materi_pokok_id = materi_pokok.id_materi_pokok", "left");
...
//END MODEL UNTUK CETAK PDF LATIHAN SOAL !!! 
//#########################################
//#########################################
//#########################################
//MODEL UNTUK SBMPTN
function fetch_profil_by_jenis_and_paket($idpaket, $jenis){
	$this->db->select("*");
	$this->db->from("profil_tryout");
...
//END MODEL UNTUK SBMPTN
//model untuk rekapitulasi bobot_soal
function fetch_konten_by_mapok($idmapok){
	$this->db->select("*");
	$this->db->from("konten_materi");
	$this->db->join("sub_materi", "konten_materi.sub_materi_id = sub_materi.id_sub_materi");
...
function fetch_jumlah_soal_by_sub($idsub){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->where("sub_materi_id", $idsub)->where("(soal.status = 0 or soal.status = 10 or soal.status = 1)");
	
	$result = $this->db->count_all_results();
	return $result;
}

function fetch_jumlah_soal_by_bobot_and_sub($idsub, $bobot){
	$this->db->select("*");
	$this->db->from("jawaban");
	$this->db->join("soal", "jawaban.soal_id = soal.id_soal");
	$this->db->where("soal.sub_materi_id", $idsub);
	$this->db->where("jawaban.bobot", $bobot)->where("(soal.status = 0 or soal.status = 10 or soal.status = 1)");
	
	$result = $this->db->count_all_results();
	return $result;
}
function fetch_soal_by_sub_materi_and_status($idsubmateri, $status){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("jawaban", "soal.id_soal = jawaban.soal_id", "left");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi", "left");
...
function fetch_soal_by_sub_materi_and_status_kur($idsubmateri, $status){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("jawaban", "soal.id_soal = jawaban.soal_id", "left");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi", "left");
...
function fetch_soal_by_sub_materi_and_status_not_kur($idsubmateri, $status){
	$this->db->select("*");
	$this->db->from("soal");
	$this->db->join("jawaban", "soal.id_soal = jawaban.soal_id", "left");
	$this->db->join("sub_materi", "soal.sub_materi_id = sub_materi.id_sub_materi", "left");
...
function update_track($idadmin, $idsoal, $status){
	$this->db->select("*");
	$this->db->from("track_qc");
	...
	if($this->db->count_all_results() == 0){
		...
		$result = $this->db->insert('track_qc', $data);
		return $result;
	}else{
		...
		$this->db->where('id_soal', $idsoal);
		$this->db->where('id_admin', $idadmin);
		$this->db->update('track_qc');
	}
}
//end model rekapitulasi bobot




//function baru untuk duplikasi cbt
function duplikat_profil(
	$kelas, $nama, $deskripsi_sub_materi,
	$penyelenggara, $tgl, $jam, $biaya, $banner, $tipe, $idpaket, $jenis, $enddate){
		$profil = array(
			'id_kelas' 		=> $kelas,
			'nama_profil' 	=> $nama,
			'penyelenggara'	=> $penyelenggara,
...
function duplikat_kategori($idprofil, $nama, $random, $tanggal, $jam, $waktu, $ketuntasan, $jumlah){
		$data = array(
			'id_profil'		=> $idprofil,
			'nama_kategori'	=> $nama,
			'random'		=> $random,
...
function fetch_soal_tryout($idkategori){
	$this->db->select("*");
...
//end function duplikasi cbt



//FUNGSI SET K13 REV
function set_k13rev($id, $k13rev){
	$data = array(
		'k_13_revisi'	=> $k13rev
	);
	$this->db->where("id_sub_materi", $id);
	$this->db->update("sub_materi", $data);

}
//END FUNGSI SET K13REV	
}
